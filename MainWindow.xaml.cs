...
namespace WinStacks
{
    public partial class MainWindow : Window
    {
        ...
        const uint SIID_COMPUTER = 61; const uint SIID_RECYCLERFULL = 33;
        ...
        {
            try {
                if (path == "WinStacks_System_ThisPC") {
                    SHSTOCKICONINFO sii = new SHSTOCKICONINFO(); sii.cbSize = (uint)Marshal.SizeOf(typeof(SHSTOCKICONINFO));
                    if (SHGetStockIconInfo(SIID_COMPUTER, SHGFI_ICON | SHGFI_LARGEICON, ref sii) == 0 && sii.hIcon != IntPtr.Zero) {
                        ImageSource img = Imaging.CreateBitmapSourceFromHIcon(sii.hIcon, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
                        DestroyIcon(sii.hIcon); return img;
                    }
                } else if (path == "WinStacks_System_RecycleBin") {
                    SHSTOCKICONINFO sii = new SHSTOCKICONINFO(); sii.cbSize = (uint)Marshal.SizeOf(typeof(SHSTOCKICONINFO));
                    if (SHGetStockIconInfo(SIID_RECYCLERFULL, SHGFI_ICON | SHGFI_LARGEICON, ref sii) == 0 && sii.hIcon != IntPtr.Zero) {
                        ImageSource img = Imaging.CreateBitmapSourceFromHIcon(sii.hIcon, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
                        DestroyIcon(sii.hIcon); return img;
                    }
                }
        ...
        private void ScanDesktopFiles()
        {
            ...
            categories["系统"].Add("WinStacks_System_ThisPC"); // 此电脑
            categories["系统"].Add("WinStacks_System_RecycleBin"); // 回收站
        ...
        private void SafeOpenItem(string path, bool isDir)
        {
            try {
                if (path == "WinStacks_System_ThisPC") {
                    Process.Start(new ProcessStartInfo { FileName = "explorer.exe", Arguments = "\"::{20D04FE0-3AEA-1069-A2D8-08002B30309D}\"", UseShellExecute = true });
                    return;
                } else if (path == "WinStacks_System_RecycleBin") {
                    Process.Start(new ProcessStartInfo { FileName = "explorer.exe", Arguments = "\"::{645FF040-5081-101B-9F08-00AA002F954E}\"", UseShellExecute = true });
                    return;
                }
